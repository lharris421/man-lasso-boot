...
\logan{What about: Posterior Augmented (PA) and Posterior Augmented Trational Hybrid (PATH)?}
...
\item For j $\in \lbrace 1, \ldots, p \rbrace$:
	...
	\item
	\begin{algorithmic}
	\If {method = Traditional $\cup$ (method = ZTAP $\cap$ $\bh_j \neq 0$)}
    \State $q_j = \bh_j$
  \ElsIf {method = Debias}
	  \State $q_j = z_j$
	\ElsIf {method = ATAP $\cup$ (method = ZTAP $\cap$ $\bh_j = 0$)}
	  \State
  	\If {$P_r \leq Post_{-}$}
...
\logan{What to put in caption vs. in text?}
