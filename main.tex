...
\pb{For this theorem, I think we should take out the $\ge$. There is no $\ge$ concept in Bayesian credible intervals -- we know the prior, so we can calculate credibility exactly.} \logan{I have no attachment to $\ge$, in the body of the proof the $\ge$ just comes from defining the credible region as such in the setup. This is just how Dan introduced it in Bayesian (with $\ge$).}
...
It is worth noting that in the above theorem we are focused on the average coverage of a single $\theta_j$ but still integrated over the joint posterior. As a direct result of the theorem, however, then the average coverage of $p$ intervals will also be equal to $1-\alpha$:

\as{
  \int \int \frac{1}{p} \sum_{j=1}^p I(\theta_j \in \cA_j(\y)) p(\y | \bt)d\y p(\bt)d\bt &= \frac{1}{p}\sum_{j=1}^p\int \int I(\theta_j \in \cA_j(\y)) p(\y | \bt)d\y p(\bt)d\bt  \\
  &= \frac{1}{p}\sum_{j=1}^p (1-\alpha) \\
  &= 1 - \alpha
}
...
\logan{I think 1 belongs and is covered below, but I added more detail for 2/3. I more meant this section just to set up the full conditional.}

What makes this formulation attractive is that quantiles can be efficiently computed from the full conditional posterior. Specifically, with a desired probability one is able to determine what tail the corresponding quantile should come from. Then, a mapping allows the original tail probability of interest from the posterior to be translated to a corresponding probability on a normal distribution (i.e. $\Norm(z_j \pm \lambda, \frac{\sigma^2}{n})$) that is related to those that make up the tails of the posterior.
...
\logan{I 100\% agree we should mention the always draw variation here... I am not sure if I see the value in bringing up the ``restricted full conditional.''}
