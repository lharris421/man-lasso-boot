...
\begin{figure}[hbtp]
  \begin{center}
    \includegraphics[width=0.35\linewidth]{ec_lasso_traditional}\includegraphics[width=0.35\linewidth]{ec_lasso_hybrid}
...
\pb{Hmm, we have a problem here. $I(\bt \in \cA)$ isn't right. That would be the indicator for whether the vector $\bt$ falls into a multivariate box $\cA$. That isn't what we're talking about. We're not creating one giant multivariate set of intervals that either contains $\bt$ or not. We're creating $p$ separate intervals, some of which will contain $\theta_j$ and others won't.}
...
\begin{proof}
Assume the likelihood and prior are correctly specified according the their respective distributions. By definition, a $100(1-\alpha)\%$ credible region for $\theta$ is any set $\cA$ s.t. $p(\theta \in \cA | \y) \geq 1 - \alpha$. For a given $\theta$, the coverage probability can be defined as $\int I(\theta \in \cA | \y) p(\y | \theta)d\y$. Then, averaged over $p(\bt)$, the average coverage probability for $p$ intervals can be defined as

\as{
  \begin{aligned}
  \int \int \frac{1}{p} \sum_{j=1}^p I(\theta_j \in \cA | \y) p(\y | \bt)d\y p(\bt)d\bt 
  &=  \int \int \frac{1}{p} \sum_{j=1}^p I(\theta_j \in \cA | \y) p(\y | \bt) p(\bt) d\y d\bt  \\
  &=  \int \int \frac{1}{p} \sum_{j=1}^p I(\theta_j \in \cA | \y) p(\bt | \y) p(\y) d\y d\bt^* \\
  &=  \int \int \frac{1}{p} \sum_{j=1}^p I(\theta_j \in \cA | \y) p(\bt | \y) d\bt p(\y) d\y \\
  &=  \int \mathbb{E}\left(\frac{1}{p} \sum_{j=1}^p I(\theta \in \cA | \y)\right) p(\y) d\y \\
  &=  \int \mathbb{E}\left(I(\theta \in \cA | \y)\right) p(\y) d\y \\
...
\logan{I still feel like there is a gap here between the thm and what we are doing with the marginal posterior... but if anything think that should be seperate?}
