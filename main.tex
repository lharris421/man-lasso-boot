...
We make the assertion that the ideal scenario for these methods is when the true distribution of $\beta$ follows a laplace (double exponential) distribution. In what follows, the reader can assume that unless otherwise noted $\beta \sim dexp(\tau = 2)$. This may raise the question of how the results change if a different rate is used. However, under a constant Signal-to-Noise ratio (SNR), the results described below are identical. \pb{would a cleaner way of saying this be: we set SNR = 1 and $\sigma^2=1$, which results in $\tau = 1.47$}.

\subsection{Interval Behavior}

\pb{best to describe overview here, something like: \\
  \\
  We begin by showing the coverage of each CI method, both overall and as the magnitude of $\beta$ changes, then present results on the width and bias of the intervals as a way of gaining insight into where the methods result in over or under coverage.}

\logan{My main concern for this section so far is making sure discussions with respect to coverage and width/bias are clearly discussed but logically seperated. Specifically, width/bias is a main explainatory piece of coverage and I want to make sure I cover what is appropriate in width/bias without awkwardly leaving holes or commenting too much that "this will be covered in the next section" in the coverage section.}

\subsubsection{Coverage}\label{Sec:Coverage}

Figure~\ref{Fig:laplace} displays simulation results for CIs obtained via the four previously described bootstrap methods. In this simulation, 100 independent datasets were generated and each bootstrap method was applied using $B = 1000$ bootstrap iterations. Each dataset was simulated as follows. $\X$ was generated independently with $n = p = 100$ and $\bb$ was generated from a Laplace distribution. Then, $\Y$ was constructed as $\Y = \X\bb + \boldsymbol{\epsilon}$, where $\boldsymbol{\epsilon} \sim N(0, \sigma^2_y)$ with $\sigma^2_y$ set such that the $SNR = 1$. The dotted lines represents the average coverage across all variables for all 100 datasets per method. The smooth lines are the fitted curves from Binomial GAMMs on the raw coverage data, estimating the coverage for a given magnitude of $\beta$. The GAMMs also include a random effect on the dataset each variable is from. \pb{I feel like we don't need the GAMM smoothing details here, only in the legend; I can't imagine it really changes too much depending on which smoothing method one uses?}
...
\logan{Is Distribution of Beta plot overkill for what we want to depict with it?}
...
\logan{Thoughts on covering slowness of blp?}
...
\subsection{Sampling from the Full Conditional Posterior}
...
through a simple mapping onto respective normal distributions.
