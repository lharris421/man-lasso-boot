...
Theorem: Assume $L(\bt|\boldsymbol{y})$ is correctly specified according to the true data generating mechanism, $p(\boldsymbol{y} | \bt)$, and that $\pi(\boldsymbol{\theta})$, the prior, matches the true distribution of $\bt$, denoted here as $p(\bt)$, then credible sets obtained from $P(\bt|\boldsymbol{y})$ will have proper coverage. \\
...
\as{
  ...
  &=  \int \int I(\bt \in \boldsymbol{A} | \boldsymbol{y}) L(\bt|\boldsymbol{y})\pi(\boldsymbol{\theta})d\boldsymbol{y}d\bt, \text{Assumed} \\
...
\logan{Should I avoid remarking on the implimentation and focus instead on the methods as much as possible? Specifically, SelInf is somewhat annoying to impliment which I could see opening up a great deal of room for error... but this is more due to choices made during implimnetation.}
