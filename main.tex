...
\item Individual vs Average Coverage (IAC) \pb{do we need these acronyms?}
\end{itemize}
...
\pb{We need to be clear here what $\lam$ was; I'm assuming that for each data set, it was $\lam_{\CV}$?}
...
\pb{For this theorem, I think we should take out the $\ge$. There is no $\ge$ concept in Bayesian credible intervals -- we know the prior, so we can calculate credibility exactly.}
...
The full conditional posterior for $\beta_j$ is defined as the distribution for $\beta_j$ conditional on $\bb_{-j} = \bbh_{-j}(\lam)$. Once $\lambda$ is selected, $\bb_{-j}$ is set equal to $\hat{\bb}_{-j}(\lambda)$ and $\r_{j}$, the partial residuals, are then defined as $\y - \X_{-j}\hat{\bb}_{-j}(\lambda)$. To simplify notation, we only explicitly condition on $\r_{j}$ to denote full conditional distributions. A normal likelihood and Laplace prior are not conjugate. However, the full conditional posterior can be shown to be a mixture of right and left truncated normals where the truncation occurs at zero for right and left tails respectively. In this manuscript, we assume that $\X$ has been standardized s.t. $\x_j^T\x_j = n$. Then, as shown in Supplement~\ref{Sup:A}, for $\beta_j$,
\as{
...
\pb{I don't have a problem with the above, but is it really simpler to write $p(\beta_j|\r_j)$ instead of $p(\beta_j|\bb_{-j})$? The latter is the more conventional way to write a full conditional.}
...
What makes this formulation attractive is that a mapping allows tail probabilities from the posterior to be translated to probabilities onto corresponding known normal distributions (i.e. $\Norm(z_j \pm \lambda, \frac{\sigma^2}{n})$). This allows for numerically stable and efficient sampling to be obtained from the full conditional posteriors.
